...
class Chat:
    ...
    def setup_chains(self):
        ...
        question_generator_prompt = PromptTemplate.from_template(prompts.QUESTION_GENERATOR_PROMPT)
        
        self.question_generator_chain = LLMChain(llm=llm, prompt=question_generator_prompt)
    ...
    def qa_response(self, message, history):
        ...
        with trace_as_chain_group("qa_response") as group_manager:
            try:
                ...
                docs = self.retriever.get_relevant_documents(search_query, callbacks=group_manager)
                doc_retrieval_logger = logging.getLogger('doc_retrieval')
                doc_retrieval_logger.info(f"Documents retrieved: {remove_non_ascii(str(docs))}")
